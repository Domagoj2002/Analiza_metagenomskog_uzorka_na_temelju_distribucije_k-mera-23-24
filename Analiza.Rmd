...
author: "0036540224 DOMAGOJ SVILIČIĆ"
date: "`r Sys.Date()`"
output:
  html_document: default
---
#Analza metagenomskog uzorka na temelju distribucije k-mera
...
column_names <- make.names(colnames(read.csv("fragments_vectors.csv", nrows = 0)), unique=FALSE)
...
# Učitavanje podataka
reference_vectors <- read_csv("reference_vectors.csv")
#fragments_vectors <- read_csv("fragments_vectors.csv")
...
gc()
fragments_vectors <- read_csv("NO_ERROR_fragments_vectors.csv")
fragments_matrix <- as.matrix(fragments_vectors)
#Oslobađanje memorije zauzete objektom fragments_vectors
rm(fragments_vectors)
gc()
...
# Klasifikacija očitanja i spremanje maksimalnih sličnosti u vektor
...
similarities_df$Color[similarities_df$Index >= 4198 & similarities_df$Index <= 4966] <- "red"
similarities_df$Color[similarities_df$Index >= 9301] <- "red"
...
# Odabir optimalnog praga
#optimal_threshold <- thresholds[which.max(f1_scores)]
#print(paste("Optimalni prag je:", optimal_threshold))
```
...
# Učitavanje podataka
reference_vectors <- read.csv("reference_vectors.csv", header = TRUE)
...
true_labels <- sub("\\.*\\d*$", "", colnames(read.csv("fragments_vectors.csv", nrows = 1)))
...
predicted_labels <- read_csv_in_chunks("fragments_vectors.csv")
...
# Učitavanje podataka
reference_vectors <- read.csv("PARTIAL_reference_vectors.csv", header = TRUE)
